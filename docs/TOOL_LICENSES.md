...
| Metasploit module metadata | — | BSD-3-Clause | `APPROVED` | **Metadata only.** `db/modules_metadata_base.json` maps modules to CVEs and carries Rapid7's reliability ranking, which is what separates "an exploit exists" from "this will be used against you". No module code is fetched or shipped. |
| **Web / API** | | | | |
