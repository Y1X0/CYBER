...
| D2 | Dynamic application security testing | `LIVE_VERIFIED` | `2d8aad1` | 141 (`test_dast_checks`, `test_dast_scanner`, `integration/test_dast_live`, `test_dast_ssrf`) | The engine was a header/cookie/TLS inspector on one response — worth having, not DAST, and unable to find an injection by construction. Now: a bounded in-scope crawler that discovers parameters and GET forms, plus **16 active checks**: reflected XSS, **stored XSS** (a marker submitted through a form, re-fetched, and found rendered unescaped on another page), SQL injection error- and boolean-based, path traversal, SSTI, OS command injection, open redirect, CORS-with-credentials, **CSRF** (a state-changing POST form with no anti-CSRF token and no SameSite=Strict/Lax session cookie — inventoried and inspected, never submitted), **session identifier in the URL**, and — **batch 1** — **blind time-based SQL injection**, **exposed sensitive paths** (`.git/`, `.env`, `.DS_Store`, backups, `/server-status` — content-signature gated, a fixed list, never brute-forced), **GraphQL introspection** (a read-only introspection query over GET), **host-header injection** (a benign `.invalid` Host reflected into a redirect or an absolute link), and **extended banner/version disclosure** (version-leaking headers beyond `Server`, reported passively). Cookie posture (Secure / HttpOnly / SameSite=Strict-or-Lax, treating `SameSite=None` as unprotective) is covered passively. Following WP-D1's precedent, no external binary — so no ZAP process, JVM, or privilege footprint, and it stays in the artifact plane. **Every payload is read-only**: no stacked SQL, no DDL, command probes `echo` a marker and nothing else, and the redirect probe targets an RFC 2606 `.invalid` host that cannot resolve. The batch-1 **time-based blind SQLi is the one bounded timing exception**: a capped `SLEEP`, sent as a control→base→doubled confirmation and reported **only when the induced delay scales proportionally with the injected sleep** — a single slow response, fixed latency, or jitter never fires it, and the confirm pair is only paid for after a base probe actually delays. GET only — a POST form is inventoried and never requested at all. State-changing paths (`/logout`, `/delete/…`) are refused before the request. Scope is re-checked in the single function that can open a socket. Live-verified over real TCP against a purpose-built vulnerable fixture: **8/8 planted flaws found, 0 findings on the 4 correct implementations of the same features** — and the scanner caught a genuine `//evil` scheme-relative bypass in the fixture's own "safe" redirect. An unreachable target now **raises** instead of returning an empty result, and a scan stopped by its budget emits an explicit coverage finding. **Deliberately not in this GET-only, unauthenticated engine:** login-endpoint rate-limit probing (needs repeated credential POSTs — a form of brute-forcing this engine refuses) and web IDOR/BOLA (needs two authenticated principals and object-by-object comparison — already implemented safely in the API-security engine, WP-D10, which takes operator-supplied credentials; guessing IDs blindly is never done). **Batch-1 items skipped here because they need a capability outside GET-only HTTP or already exist elsewhere:** subdomain takeover (subdomain enumeration + DNS CNAME resolution — the discovery `dns_provider` already flags dangling-CNAME NXDOMAIN); TLS/cipher weakness (a raw TLS handshake — the `web_tls` provider already flags certificate expiry and hostname mismatch); email SPF/DKIM/DMARC (DNS TXT lookups — the `dns_posture` provider already covers SPF and DMARC, and DKIM needs selector brute-forcing); and `security.txt` presence (already flagged by the `security-txt-missing` web-checks template). |
