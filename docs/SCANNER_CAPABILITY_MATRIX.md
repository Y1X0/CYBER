...
| — | **SBOM (CycloneDX 1.5)** | `repo`, `container_image`, `server_host`, `mobile_app`, `ios_app` | `sca`, `container`, `host_posture`, `mobile`, `ios` | IMPLEMENTED, multi-asset | One SBOM per scan, **merged** from every engine that enumerates components (source deps, image packages, server packages via the agent, Android native libs, iOS frameworks). Any engine exposing `collect_inventory()` contributes; no separate SBOM system. Downloadable, tenant-isolated, vuln-cross-referenced — see docs/SBOM.md. |
